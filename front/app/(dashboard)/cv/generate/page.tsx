...
export default function CVGeneratePage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Notice */}
      <div className="rounded-2xl border border-amber-800/30 bg-amber-500/10 p-4 text-sm text-amber-300">
      ...
      {cv && (
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-xl">
          ...
          <div className="mb-6">
            ...
            {cv.cv.about && (
              <p className="mt-3 text-sm text-zinc-400">{cv.cv.about}</p>
            )}
          </div>
              ...
              <div className="mt-2 space-y-3">
                {cv.education.map((edu) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-zinc-400">
                      {edu.institution} · {edu.start_date ?? "—"} — {edu.end_date ?? "Presente"}
                    </p>
                    {edu.description && (
                      <p className="mt-1 text-sm text-zinc-500">{edu.description}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 shadow-xl">
        ...
        <div className="flex gap-3">
          <button
            disabled
            className="rounded-xl border border-zinc-700 bg-zinc-800/60 px-6 py-3 text-sm font-medium text-zinc-500 cursor-not-allowed"
          >
            Generar PDF
          </button>
          <button
      ...
      <div className="flex justify-start">
        <Link
          href="/cv/edit"
          className="rounded-xl border border-zinc-700 bg-zinc-800/60 px-4 py-2 text-sm font-medium text-zinc-300 hover:border-indigo-500/30 hover:text-white transition-all"
        >
          ← Volver al editor
        </Link>
      </div>
